...
//  run this as as super user: sudo su;
...
#include <unistd.h>
#include <math.h>
#include <sched.h> // contains SCHED_FIFO and others
#include <pthread.h>
#include <stdio.h>
#include <sys/types.h>
...
int main()
{
    ...
    periods[0] = 100 * USEC; // This has to be done in nanoseconds
    ...
    if (getuid() == 0)
        pthread_setschedparam(pthread_self(), SCHED_FIFO, &priomax); // Setting highest priority to the current thread
    ...
    /* FIXME: Would be nice to add a function that is checking if we have a harmonic relationship
     * But for now we will assume U_lub = 1;
     * as we clearly see our periods to be in this relationship
    ...
    iret[1] = pthread_create(&(thread_id[0]), &(attributes[0]), task2, NULL);
    iret[2] = pthread_create(&(thread_id[0]), &(attributes[0]), task3, NULL);
...
void *task1(void *ptr)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        // TODO: it would be nice to check if we missed a deadline here... why don't you try by yourself?
