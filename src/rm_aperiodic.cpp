...
// DISCLAIMER THIS IS NOT WORKING PROPERLY AS IT IS USING 
// FUNCTIONS FROM TWO DIFFERENT VERSIONS OF LINUX KERNEL
...
int main()
{
	...
	// aperiodic tasks
	for (i = NPERIODICTASKS; i < NTASKS; i++)
	{
		// initializa the attribute structure of task i
		pthread_attr_init(&(attributes[i]));
		pthread_attr_setschedpolicy(&(attributes[i]), SCHED_FIFO);

		// set minimum priority (bacground scheduling)
		parameters[i].sched_priority = 0; // Minimum priority = 0 == background scheduling
	}
...
void task1_code()
{
	...
	// when the random variable ap=0, then aperiodic task 5 must
	// be executed
	if (ap == 0)
	{
		printf(":Execute (4)");
